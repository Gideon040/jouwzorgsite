...
export function QuoteSection({ style, theme, palette, content, generated }: QuoteSectionProps) {
  ...
  // Deterministic selection (Math.random causes hydration mismatch)
  const nameHash = (content.naam || '').length + (content.tagline || '').length;
  const quote = DEFAULT_QUOTES[nameHash % DEFAULT_QUOTES.length];
