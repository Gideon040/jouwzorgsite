...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// DOMEIN BEHEER (als er al een domein gekoppeld is)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

export function DomeinBeheer({ siteId, domain }: { siteId: string; domain: string }) {
  const [retrying, setRetrying] = useState(false);
  const [disconnecting, setDisconnecting] = useState(false);
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  const [disconnected, setDisconnected] = useState(false);

  const handleRetryVercel = async () => {
    setRetrying(true);
    setMessage(null);

    try {
      const res = await fetch('/api/domains/retry-vercel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        setMessage({ type: 'error', text: data.error || 'Kon niet opnieuw koppelen' });
      } else {
  ...
  const handleDisconnect = async () => {
    if (!confirm('Weet je zeker dat je dit domein wilt loskoppelen?')) return;
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({ type: 'error', text: data.error || 'Kon domein niet loskoppelen' });
      } else {
  ...
  if (disconnected) {
    return (
      <div className="bg-white rounded-xl border border-slate-200/60 p-6 text-center">
        ...
        <h3 className="text-sm font-semibold text-slate-900 mb-1">Domein losgekoppeld</h3>
        <p className="text-xs text-slate-500 mb-4">
          Herlaad de pagina om een nieuw domein te koppelen.
        </p>
        <button
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200/60 p-6 space-y-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-slate-900">Domein beheren</h3>
        </div>

        <p className="text-sm text-slate-600">
          Je domein <strong>{domain}</strong> is gekoppeld. Zorg dat je DNS-records correct zijn ingesteld:
        </p>
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {retrying ? 'Bezig...' : 'Opnieuw koppelen aan Vercel'}
          </button>
          <button
          ...
          >
            {disconnecting ? 'Bezig...' : 'Loskoppelen'}
          </button>
        </div>
      </div>
    </div>
